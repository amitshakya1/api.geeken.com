...
export class UsersService {
  ...
  async create(
    ...
    if (createUserDto.email) {
      queryBuilderEmail.andWhere("user.email = :email", {
        email: createUserDto.email,
      });
    }
    if (createUserDto.role) {
      queryBuilderEmail.andWhere("role.name = :role", {
    ...
    if (existingUser) {
      throw new ConflictException(
        `User with this ${createUserDto.role ? "role and" : ""} email already exists`
      );
    }
    ...
    if (createUserDto.phone) {
      queryBuilderPhone.andWhere("user.phone = :phone", {
        phone: createUserDto.phone,
      });
    }
    if (createUserDto.role) {
      queryBuilderPhone.andWhere("role.name = :role", {
        role: createUserDto.role,
      });
    }
    // Check for unique email
    ...
    if (existingPhone) {
      throw new ConflictException(
        `User with this ${createUserDto.role ? "role and" : ""} phone already exists`
      );
    }
  ...
  async update(id: string, updateUserDto: UpdateUserDto): Promise<User> {
              ...
              company.bankAccounts = await Promise.all(
                companyDto.bankAccounts.map(async (bank) => {
                  if (bank.id) {
                    ...
                    const bankAccount =
                      this.companyBankAccountRepository.create({
                        ...existing,
                        ...bank,
                        company,
                      });
                    return await this.companyBankAccountRepository.save(
                      bankAccount
                    );
                  } else {
                    const bankAccount =
              ...
              company.documents = await Promise.all(
                companyDto.documents.map(async (doc) => {
                  ...
                  if (doc.id) {
                    ...
                    return await this.companyDocumentRepository.save(
                      companyDocument
                    );
                  } else {
                    const companyDocument =
    ...
    const { imageId, fileIds, companies, documents, ...updateData } =
      updateUserDto;

    if (imageId !== undefined) {
      user.image = imageId ? ({ id: imageId } as any) : null;
    }
